...
from config import parse_signal_args
import numpy as np
from models.TSTranformer import *
import os
...
def test(test_data):
    ...
    FNv = 0
    with torch.no_grad():
        for i, (xv, yv, zv, sv, labelv) in enumerate(test_data):
            xv = xv.cuda()
            yv = yv.cuda()
            zv = zv.cuda()
            sv = sv.cuda()
            output = model(xv, zv, yv, sv)
            labelv = labelv.cuda()
...
def train(train_data):
    ...
    TN = 0
    FN = 0
    for i, (x, y, z, s, label) in enumerate(train_data):
        ...
        correct = correct + ((predicted.eq(label).sum().item()) / (len(label)*len(label[0])))
